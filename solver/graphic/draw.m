function [handles_left, handles_middle, handles_right] = draw(faces)
    %init stuff
    clf;
    ...
    for x = 0:2
        for y = 0:2
            for z = 0:2
                ...
                if x == 0
                    handles_left(y+1, z+1) = drawcube(vert, fac, colors);
                elseif x == 1
                    handles_middle(y+1, z+1) = drawcube(vert, fac, colors);
                else 
                    handles_right(y+1, z+1) = drawcube(vert, fac, colors);
                end
            end
        end
    end    
...
function handle = drawcube (vert, fac, colors)
    handle = patch('Vertices',vert,'Faces',fac, 'FaceVertexCData',colors,'FaceColor','flat');
end